...
function M.toggle_obs_daily()
  local buf = vim.api.nvim_get_current_buf()
  local note_exists = false
  local buf_name = vim.api.nvim_buf_get_name(buf)

  local function close_tab_if_multiple()
    if (#vim.api.nvim_list_tabpages()) > 1 then
      vim.cmd 'tabclose $'
    end
  end

  if buf_name == cur_daily.path then
    if vim.bo.modified then
      p 'SAVING...'
      vim.cmd.write()
    end
    require('custom.util').bufdelete(buf)
    close_tab_if_multiple()
    return
  -- Check if already opened in another window/buffer
  else
    for _, b in ipairs(vim.api.nvim_list_bufs()) do
      if vim.api.nvim_buf_is_loaded(b) and vim.api.nvim_buf_get_name(b) == cur_daily.path then
        buf = b
        note_exists = true
        break
      end
    ...
    -- if not open, open a new one
      ...
      if is_existing then
        vim.cmd('tab split ' .. cur_daily.path)
      else
...
-- get & save cursor position on first bufread
